                    ...
                    onSelect={() => {
                      if (multiSelect) {
                        const updatedValues = isSelected
                          ? selectedValues.filter((val) => val !== option.value) //存在则删除
                          : [...selectedValues, option.value]; //不存在则添加
                        onFilterChange(updatedValues);
                      } else {
                        onFilterChange(isSelected ? [] : [option.value]); //只能选一个
                        onClose();
                      }
                    }}
                  >
            ...
            {selectedValueSet.size > 0 && (
              <>
                ...
                <CommandGroup className="sticky bottom-0 align-bottom bg-white">
                  <CommandItem
                    onSelect={() => onFilterChange([])} //清除所有过滤器的值
